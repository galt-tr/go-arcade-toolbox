...
| **Connection pool** | `perfprovider.Config.MaxDBConns` | PostgreSQL only. The shared pool serves workers **and** the monitor. Too low → workers queue on connections; too high → PostgreSQL context-switches. Benchmarks used `conns ≈ workers + a margin`. Ignored on SQLite, which stays pinned to one writer connection: it serializes writes, so a wider pool yields `SQLITE_BUSY`, not parallelism. |
