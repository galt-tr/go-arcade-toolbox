...
//
// GENERATOR DRIFT: tools/client-gen does not exist in this repository, so the
// "DO NOT EDIT" above is aspirational — this file is HAND-MAINTAINED. Edit it
// directly, and keep the read/write split in sync with the @Read/@Write
// annotations on [wdk.WalletStorageProvider]:
//
//   - every @Write EXCEPT MakeAvailable and SetActive goes through
//     getActiveWriter, which can REFUSE (see ErrStorageNotActive). Those two
//     are deliberately ungated and hand-written in storage_manager.go —
//     MakeAvailable establishes the user selection the gate reads, and
//     SetActive is the only way back out of a refusal.
//   - every @Read goes through getActiveReader, which never refuses.
//
// REGENERATION TRAP: the //go:generate directives in
// pkg/wdk/storage.interface.go are live text (currently inert only because the
// generator is unported). The manager.tpl line already lists MakeAvailable and
// SetActive in -skip-methods, but a naive `go generate` once client-gen lands
// would overwrite this file from a template that knows nothing about the write
// gate, silently deleting the P1-2 double-spend guard. Port the gate into
// manager.tpl BEFORE running that directive.
...
func (m *WalletStorageManager) Migrate(ctx context.Context, storageName string, storageIdentityKey string) (string, error) {
	w, err := m.getActiveWriter()
	if err != nil {
		return "", err
	}

	return w.Migrate(ctx, storageName, storageIdentityKey)
}
...
func (m *WalletStorageManager) FindOrInsertUser(ctx context.Context, identityKey string) (*wdk.FindOrInsertUserResponse, error) {
	...
	return w.FindOrInsertUser(ctx, identityKey)
}
...
func (m *WalletStorageManager) InternalizeAction(ctx context.Context, args wdk.InternalizeActionArgs) (*wdk.InternalizeActionResult, error) {
	...
	return w.InternalizeAction(ctx, auth, args)
}
...
func (m *WalletStorageManager) CreateAction(ctx context.Context, args wdk.ValidCreateActionArgs) (*wdk.StorageCreateActionResult, error) {
	...
	return w.CreateAction(ctx, auth, args)
}
...
func (m *WalletStorageManager) ProcessAction(ctx context.Context, args wdk.ProcessActionArgs) (*wdk.ProcessActionResult, error) {
	...
	return w.ProcessAction(ctx, auth, args)
}
...
func (m *WalletStorageManager) InsertCertificateAuth(ctx context.Context, certificate *wdk.TableCertificateX) (uint, error) {
	...
	return w.InsertCertificateAuth(ctx, auth, certificate)
}
...
func (m *WalletStorageManager) RelinquishCertificate(ctx context.Context, args wdk.RelinquishCertificateArgs) error {
	...
	return w.RelinquishCertificate(ctx, auth, args)
}
...
func (m *WalletStorageManager) RelinquishOutput(ctx context.Context, args wdk.RelinquishOutputArgs) error {
	...
	return w.RelinquishOutput(ctx, auth, args)
}
...
func (m *WalletStorageManager) AbortAction(ctx context.Context, args wdk.AbortActionArgs) (*wdk.AbortActionResult, error) {
	...
	return w.AbortAction(ctx, auth, args)
}
