...
// NOTE: the tools/client-gen generator referenced below has not been ported to
// this repository yet; these directives are inert until it is ported in a
// later milestone. The mockgen directive is likewise inert for a separate
// reason: `go tool mockgen` requires a `tool` block registration for
// go.uber.org/mock/mockgen in go.mod, which is currently absent.
...
//go:generate go tool mockgen -destination=../internal/mocks/mock_wallet_storage_writer.go -package=mocks github.com/bsv-blockchain/go-arcade-toolbox/pkg/wdk WalletStorageProvider
